...
var colors = [
    new THREE.Color(0x60A65F),
    new THREE.Color(0xFFEC97),
    new THREE.Color(0x247065)];
...
loader.crossOrigin = "";
var dotTexture = loader.load("https://cdn.rawgit.com/MattiaAMG/amg-website/main/dots-bg/img/dotTexture.png");
var dotsAmount = 2000;
...
for (i = dotsGeometry.vertices.length - 1; i >= 0; i--) {
    ...
    for (var j = dotsGeometry.vertices.length - 1; j >= 0; j--) {
        if (i !== j && vector.distanceTo(dotsGeometry.vertices[j]) < 10) {
...
function render(a) {
     // Update rotation
    galaxy.rotation.y += 0.000005; // Adjust the rotation speed as desired
    wrap.rotation.y += 0.000005; // Adjust the rotation speed as desired
